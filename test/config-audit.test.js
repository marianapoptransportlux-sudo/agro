...
test("createConfigEntry rejects new user with password shorter than 10 characters", async () => {
  await withIsolatedWorkspace(async ({ load }) => {
    const storage = load("src/local-storage.js");
    ...
    await assert.rejects(
      ...
      /minim 10 caractere/i
    );
  });
});

test("createConfigEntry accepts new user with password of 10+ characters", async () => {
  await withIsolatedWorkspace(async ({ load }) => {
    const storage = load("src/local-storage.js");
...
test("updateConfigEntry rejects password change shorter than 10 characters", async () => {
  await withIsolatedWorkspace(async ({ load }) => {
    const storage = load("src/local-storage.js");
